...
#3.Combine MultipleGROUPBYs and  target and Reference aggregates
'''SELECT 
    (a1, a2, ... an),
    (f1_m1, f1_m2, ... f1_mn, ... f2_m1, f2_m2, ... fn_mn)
FROM 
...
#SortedViewScores=EvaluationViews()
...
def top_5_AggViews(ranking):
    for vid in ranking[:5]:
        ...
        utils.display_Graph(tgt_dict, ref_dict, (a, m, f))
...
top_5_AggViews([rank[0] for rank in top_5_views])
#top_5_AggViews([r[0] for r in SortedViewScores])
