...
#1.Combining multiple aggregates
def combineMultipleAggregates():
    ...
    return res_married,res_unmarried
...
res_married,res_unmarried=combineMultipleAggregates()


# 2. Combining target and reference view query
...
#3.Combine MultipleGROUPBYs and aggregates
'''SELECT 
    (a1, a2, ... an),
    (f1_m1, f1_m2, ... f1_mn, ... f2_m1, f2_m2, ... fn_mn)
FROM 
...
'''
def combiningMultipleGroupby():

    Married_query = 'select ' + ','.join(dimension_attr)
    Married_query+=aggregate_query +  ' from married_adults where'
    Married_query+=where_married +  ' group by grouping sets( '+ ','.join(dimension_attr)+' ) ;'
    cursor.execute(Married_query)
    resMarried = pd.DataFrame(cursor.fetchall())
    colsMarried= cursor.description

    Unmarried_query = 'select ' + ','.join(dimension_attr)
    Unmarried_query+=aggregate_query +  ' from unmarried_adults where'
    Unmarried_query+=where_married +  ' group by grouping sets( '+ ','.join(dimension_attr)+' ) ;'
    cursor.execute(Unmarried_query)
    resUnmarried= pd.DataFrame(cursor.fetchall())
    colsUnmarried= cursor.description

    resMarried.columns = [col[0] for col in colsMarried]
    resUnmarried.columns = [col[0] for col in colsUnmarried]

    return resMarried,resUnmarried


resMarried,resUnmarried=combiningMultipleGroupby()

print("resMarried:",resMarried)
print("Unmarried:",resUnmarried)
